...
⚫️MVPリリース時に実装したい機能
[レ]•ユーザー登録・ログイン機能
[レ]•目標設定ができる機能  　　　　　　　　　　　　　　　　# React使用予定 (ユーザーが目標を設定するためのフォーム、フォームのバリデーションやインタラクティブなUIをReactで実装)
[レ]•トレーニングメニューを提案する機能
[レ]•筋トレメニューのカスタマイズ機能  　　　　　　　　　　# React使用予定 (動的なフォームやリアルタイムのフィードバック、ユーザーがトレーニングメニューをカスタマイズするためのインターフェースをReactで実装、)
[レ]•トレーニング記録の登録・表示機能（カレンダー形式）  　# React使用予定 (カレンダーのUIをReactで実装、FullCalendar.jsなどのライブラリを使用して、Reactコンポーネントとしてカレンダーを表示)
[レ]•基礎代謝を計算させる機能
[レ]•1日の歩数を登録・表示・消費カロリーの計算機能
[レ]•1日の消費カロリーを算出させる機能
[レ]•体重管理機能（記録をグラフで表示）# 折れ線グラフで表示
[レ]•プライバシー規約の表示機能
[レ]•お問い合わせフォーム機能

⚫️本リリースまでに追加したい機能
[レ]•トレーニング種目を登録した際に、種目の横に、その種目のMAX重量もしくはMAX回数が表示される機能
[レ]•ダイエットについて説明するページを追加
[レ]•トレーニング時間の計測とアラート通知する機能
[レ]•摂取カロリーを記録させる機能
[レ]•摂取カロリーと消費カロリーをもとに、1日のカロリーが摂り過ぎなのかを判断させる機能
[レ]•トレーニングメニューや感想などを投稿（共有）する機能
[レ]•投稿された内容に対して、いいねとコメントができる機能
[レ]•誰からいいねされたのか、投稿者にわかるように通知される機能
[レ]•トレーニングや消費カロリーの結果をXへ共有できる機能
...
タイマー機能: JavaScriptのsetTimeout、Audioオブジェクト
...
メール送信: Action Mailer
API連携: omniauth-twitter Gem


インフラ技術
クラウドプロバイダー: Render
サーバー: Render Web Services
