              ...
              item.exercises.map((exercise, i) => (
                <div key={i}>
                  {typeof exercise === "string"
                    ? exercise
                    : `${exercise.name} ${exercise.reps ? `${exercise.reps}回` : ""}×${exercise.sets ? `${exercise.sets}セット` : ""}`}
                </div>
              ))
            ) : (
      ...
      if (frequency === "6回/週") {
        if (volume === "多いのがいい！") {
            ...
            { day: "土曜日", exercises: [{ name: "クランチ", reps: 20, sets: 5 }, { name: "レッグレイズ", reps: 20, sets: 5 }] },
              ...
              ...training,
              exercises: training.exercises.map(exercise => `${exercise.name} ${exercise.reps}回×${exercise.sets}セット`)
            })));
