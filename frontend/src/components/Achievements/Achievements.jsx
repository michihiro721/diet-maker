...
const Achievements = () => {
  ...
  }, [userId, selectedDate, fetchTrainingData]);
  ...
  const groupTrainingDataByCategory = () => {
    return trainingData.reduce((groups, training) => {
  ...
  const renderTrainingTable = (groupedTrainingData) => {
    return (
      <table className="ach-training-records-table">
        ...
        <tbody>
          {renderTrainingTableRows(groupedTrainingData)}
        </tbody>
      </table>
    );
  };
  ...
  const renderTrainingTableRows = (groupedTrainingData) => {
    return Object.entries(groupedTrainingData).map(([category, trainings]) => {
  ...
  const renderTrainingRecords = () => {
    const groupedTrainingData = groupTrainingDataByCategory();
        ...
        {trainingData.length > 0 ? (
          <div className="ach-training-records-by-category">
            <div className="ach-category-section">
              {renderCategoryBadges(Object.keys(groupedTrainingData))}
              {renderTrainingTable(groupedTrainingData)}
            </div>
          </div>
        ) : (
